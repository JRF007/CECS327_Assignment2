...
1) RPC subscribe(lotId) -> subId
2) Connect to event_port and send: SUB <subId>\n
3) Print incoming EVENT lines.

Example:
  python3 subscriber_client.py --host 127.0.0.1 --rpc-port 5001 --event-port 5003 --lot A
"""
...
def main():
    ...
    rpc = RPCClient((args.host, args.rpc_port), timeout=2.0)
    sub_id = rpc.subscribe(args.lot)
    print(f"subscribed lot={args.lot} subId={sub_id}", flush=True)

    ev = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
